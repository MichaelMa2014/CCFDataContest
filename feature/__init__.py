...
def build_vectorizer(rows=None):
    ...
    path = os.path.abspath('{temp}/vectorizer.model'.format(temp=conf.TEMP_DIR))
...
def build_tokenizer(rows=None):
    ...
    if not os.path.exists(conf.TEMP_DIR):
        os.mkdir(conf.TEMP_DIR)
    path = os.path.abspath('{temp}/tokenizer.model'.format(temp=conf.TEMP_DIR))
    if not os.path.exists(path):
        if rows is None:
            rows = data.load_train('query')
            rows = data.process_data(rows, remove_stopwords=False)
...
def build_word2vec_model(word_vec_dim):
    ...
    if not os.path.exists(conf.TEMP_DIR):
        os.mkdir(conf.TEMP_DIR)
    path = os.path.abspath('{temp}/word2vec_dim{dim}.model'.format(
        temp=conf.TEMP_DIR, dim=word_vec_dim))
    if not os.path.exists(path):
...
def build_weights_matrix(word_vec_dim):
    ...
    if not os.path.exists(conf.TEMP_DIR):
        os.mkdir(conf.TEMP_DIR)
    path = os.path.abspath('{temp}/weights_dim{dim}.npy'.format(
        temp=conf.TEMP_DIR, dim=word_vec_dim))
    if not os.path.exists(path):
    ...
    weights = numpy.load(path, mmap_mode='r+')  # TODO maybe shouldn't use mmap
    util.init_random()
    return weights
