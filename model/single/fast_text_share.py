...
import conf
import feature.wv_share
import submissions
import util

_file_name = os.path.splitext(os.path.basename(__file__))[0]
param = {'sparse': False, 'batch_size': 128, 'nb_epoch': 60}


def build_clf(input_dim, output_dims, max_feature, word_vec_dim=100,
...
def build():
    ...
    构建分类器
    """
    X_train, y_train, X_val, y_val, y_shape, max_feature = \
        feature.wv_share.build_train(validation_split=0.1,
                                     sparse=param['sparse'])
    y_train_split = []
    y_val_split = []
    ...
    clf = build_clf(X_train.shape[1], y_shape, max_feature,
    ...
    clf.fit(X_train, y_train, batch_size=param['batch_size'],
            nb_epoch=param['nb_epoch'], validation_data=(X_val, y_val),
...
def run():
    ...
    X_test = feature.wv_share.build_test(sparse=param['sparse'])
